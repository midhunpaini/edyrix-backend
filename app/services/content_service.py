...
async def get_subject_detail(
    ...
    total_lessons = sum(ch.lessons_total for ch in chapter_summaries)
    completed_lessons = sum(ch.lessons_completed for ch in chapter_summaries)
    completed_chapters = sum(
        1 for ch in chapter_summaries if ch.lessons_total > 0 and ch.lessons_completed == ch.lessons_total
    )
    subject_progress_pct = (
        round((completed_lessons / total_lessons) * 100, 2) if total_lessons else 0.0
    )
    estimated_hours = round(sum(ch.estimated_minutes for ch in chapter_summaries) / 60, 2)
    ...
    return SubjectDetailResponse(
        ...
        monthly_price_paise=subject.monthly_price_paise,
        has_access=has_access,
        watch_percentage=round(subject_progress_pct),
...
async def is_lesson_unlocked(
    ...
    ordered = sorted(chapter_lessons, key=lambda item: (item.order_index, item.created_at, item.id))
    current_index = next((idx for idx, item in enumerate(ordered) if item.id == lesson.id), -1)
    if current_index <= 0:
        return True

    previous_lesson = ordered[current_index - 1]
    previous_completed_result = await db.execute(
        select(WatchHistory.id).where(
            WatchHistory.user_id == user_id,
            WatchHistory.lesson_id == previous_lesson.id,
            WatchHistory.is_completed.is_(True),
        ).limit(1)
    )
    return previous_completed_result.scalar_one_or_none() is not None
...
async def is_test_unlocked(
    ...
    completed_count = int(completed_count_result.scalar() or 0)
    ...
    return watched_any_result.scalar_one_or_none() is not None
...
async def get_chapter_detail(
    ...
    chapter_test_result = await db.execute(
        select(Test)
        .where(
    ...
    chapter_test = chapter_test_result.scalar_one_or_none()
    chapter_test_id = chapter_test.id if chapter_test else None
    ...
    if chapter_test_id is not None:
        ...
        last_attempt_result = await db.execute(
            ...
            .where(
                TestAttempt.user_id == user.id,
                TestAttempt.test_id == chapter_test_id,
            )
            .order_by(TestAttempt.completed_at.desc())
            .limit(1)
        )
        chapter_last_attempt = last_attempt_result.scalar_one_or_none()
        if chapter_last_attempt is not None and chapter_last_attempt.percentage is not None:
            chapter_test_last_score = float(chapter_last_attempt.percentage)
    ...
    for lesson_number, lesson in enumerate(lessons, start=1):
        ...
        unlocked_by_sequence = await is_lesson_unlocked(
            db,
            user.id,
            lesson,
            lessons,
        )
        unlocked = lesson.is_free or (has_access and unlocked_by_sequence)

        test_summary: LessonTestSummary | None = None
        if test is not None:
            test_unlocked = chapter_test_unlocked and unlocked
            unlock_reason = None
            if not lesson.is_free and not has_access:
                unlock_reason = "subscription_required"
            elif not test_unlocked:
        ...
        lesson_summaries.append(
            LessonSummary(
                ...
                can_take_test=chapter_test_id is not None and is_completed,
                test_last_score=chapter_test_last_score,
                test=test_summary,
            )
        )
    ...
    return ChapterDetailResponse(
        ...
        has_notes=has_notes,
        has_test=chapter_test_id is not None,
        test_id=chapter_test_id,
    )
...
async def _chapter_summary(
    ...
    lesson_rows_result = await db.execute(
        select(Lesson.id, Lesson.duration_seconds).where(
            Lesson.chapter_id == chapter.id,
            Lesson.is_published.is_(True),
        )
    )
    lesson_rows = lesson_rows_result.all()
    lesson_ids = [row.id for row in lesson_rows]
    lessons_total = len(lesson_ids)
    total_duration_seconds = sum(int(row.duration_seconds or 0) for row in lesson_rows)
    estimated_minutes = round(total_duration_seconds / 60) if total_duration_seconds else 0
    ...
    if lesson_ids:
        ...
        lessons_completed = int(completed_result.scalar() or 0)
        ...
        has_started = lessons_completed > 0 or int(started_result.scalar() or 0) > 0
        is_completed = lessons_total > 0 and lessons_completed == lessons_total

    progress_pct = round((lessons_completed / lessons_total) * 100, 2) if lessons_total else 0.0
    ...
    return ChapterSummary(
        ...
        lesson_count=lessons_total,
        has_test=has_test,
        watch_percentage=round((lessons_completed / lessons_total) * 100) if lessons_total else 0,
        is_completed=is_completed,
    )
